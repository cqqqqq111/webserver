...
int main(){
    ...
    socklen_t clnt_addr_len=sizeof(clnt_addr);//记录客户端地址长度
    bzero(&clnt_addr,sizeof(clnt_addr));//server不仅需要表示自己的地址 还需要一个结构体表示客户端地址
    
    int clnt_sockfd=accept(sockfd,(sockaddr*)&clnt_addr,&clnt_addr_len);//阻塞等待客户端连接，收到客户端信息才会向下进行
